...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    PriorityQueue<Path> paths = new PriorityQueue<Path>(); // PQueue to hold intermediate paths
    Vertex startVertex = vertices.get(start);
    Vertex endVertex = vertices.get(end);
    if (startVertex == null || endVertex == null) { // makes sure start an end vertices are in graph
      throw new NoSuchElementException();
    }
    Path startPath = new Path(startVertex);
    paths.add(startPath);
    LinkedList<Vertex> discovered = new LinkedList<Vertex>(); // holds vertices whose paths have
                                                              // been discovered
    while (!paths.isEmpty()) {
      Path current = paths.peek(); // head is used
      discovered.add(current.end); // shortest path for current vertex has been found
      for (Edge edge : current.end.edgesLeaving) { // add each edge leaving to the PQueue
        if (!discovered.contains(edge.target)) { // don't add if target vertex is discovered
          Path toExtend = new Path(current);
          toExtend.extend(edge); // extend a copy of the previous path
          paths.add(toExtend); // add it to the PQueue
        }
      }
      if (paths.peek() != null && paths.peek().end.equals(endVertex)) {
        return paths.poll(); // return head if it has end vertex
      }
      paths.poll(); // remove head
    }
    throw new NoSuchElementException(); // path not found from start to end
  }
