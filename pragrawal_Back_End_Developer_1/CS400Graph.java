...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
          ...
          dataSequence.addAll(source.dataSequence);
          end = source.end;
          }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
      ...
      LinkedList<Vertex> visited = new LinkedList<Vertex>();    // To keep track of visited nodes
      Vertex sourceVertex = this.vertices.get(start);
      Vertex targetVertex = this.vertices.get(end);
      if(sourceVertex == null || targetVertex == null) throw new NoSuchElementException();
      Path shortest = new Path(sourceVertex);   // Path object for no path found
      Path sourcePath = new Path(sourceVertex); // Initial path of source
      // distance is the cost of the path
      sourcePath.distance = 0;  // source vertex's path to itself
      if(start.equals(end)) return shortest;    // if start = end then shortest path is to itself
      pq.add(sourcePath);
      // Loop till pq is empty
      while(!pq.isEmpty()) {
        Path currentPath = pq.poll();   // Path with lowest cost in pq
        Vertex frontier = currentPath.end;  // Path target is the frontier
        visited.add(frontier);    // Mark visited by adding to the list
        if(frontier.equals(targetVertex)) { // Return when target vertex is reached
          shortest = currentPath;
          return currentPath;
        }
        // Traverse all edges leaving the vertex
        for(int i = 0; i < frontier.edgesLeaving.size(); i++) {
          Path nextPath = new Path(currentPath);   // Indivisual paths to the next possible vertex
          nextPath.end = frontier.edgesLeaving.get(i).target;
          if(!visited.contains(nextPath.end)) {   // Add frontier if not already visited
            //    nextPath: currentPath + frontier -> target
            nextPath.extend(new Edge(nextPath.end, frontier.edgesLeaving.get(i).weight));
            pq.add(nextPath); // Add to pq
          }
        }
      }
      //  if path starts and end on the source vertex then the path hasn't been found
      if(shortest.start == shortest.end) throw new NoSuchElementException("Path cannot be found.");
